      ...
      try {
        ...
        switch (effectiveAction) {
          ...
          case 'get_profile': {
            ...
            let query = supabase.from('influencers').select('*').eq('user_id', userId);
            if (isNumeric) {
          ...
          case 'sync_intelligence': {
            const targetUrl = body.url || rawUsername;
            if (!targetUrl) throw new Error('Refresh Error: Missing "username" or "url".');

            const targetUsernameClean = extractUsername(targetUrl);
            console.log(`[POST] refresh_profile → direct scrape for: ${targetUsernameClean}`);

            // --- SMART CHECK: Is the profile already in DB and fresh? ---
            let existingProfile = null;
            if (userId && targetUsernameClean !== 'unknown') {
              const { data: existing } = await supabase
                .from('influencers')
                .select('*')
                .eq('user_id', userId)
                .eq('username', targetUsernameClean)
                .maybeSingle();
              existingProfile = existing;
            }

            // If profile exists and is fresh (< 30 min), return immediately from DB
            if (existingProfile && existingProfile.last_updated_at && !body.force) {
              const lastUpdate = new Date(existingProfile.last_updated_at).getTime();
              const thirtyMinutesAgo = Date.now() - (30 * 60 * 1000);
              if (lastUpdate > thirtyMinutesAgo) {
                console.log(`[POST] Profile ${targetUsernameClean} is fresh. Returning from DB.`);
                return new Response(JSON.stringify({
                  success: true,
                  data: existingProfile,
                  message: 'Profile is up to date (refreshed < 30 min ago).'
                }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 });
              }
            }

            // --- DIRECT SCRAPE: Call /scrape which handles both new and existing profiles ---
            console.log(`[POST] Calling master scrape node for: ${targetUsernameClean}`);
            const { data: scrapeRes, error: scrapeErr } = await supabase.functions.invoke('scrape', {
              body: {
                url: targetUrl,
                force: true,
                userId: userId,
            ...
            if (scrapeErr) throw new Error(`Scrape Node Error: ${scrapeErr.message}`);
            if (!scrapeRes?.success) throw new Error(scrapeRes?.error || 'Scrape node returned failure with no error message.');

            // Scrape gives us back the influencer row directly
            let finalData = scrapeRes?.data;

            // Final DB fallback if scrape didn't return the row
            if (!finalData && userId && targetUsernameClean !== 'unknown') {
            ...
            if (!finalData) throw new Error('Sync completed but profile could not be resolved. Check Supabase logs.');

            return new Response(JSON.stringify({
              success: true,
              data: finalData,
              message: scrapeRes?.message || 'Intelligence synchronized successfully.'
            }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 });
          }
          ...
          case 'sync_reels': {
            ...
            // --- NEW UNIFIED DELEGATION: Call the dedicated scrape function ---
            ...
            if (scrapeError) {
              console.error(`[POST] Delegation Error for ${targetUsername}:`, scrapeError);
              throw new Error(`Sync Delegation Failure: ${scrapeError.message || 'Internal Scraper Error'}`);
            }

            if (!scrapeData?.success) {
              throw new Error(scrapeData?.error || 'Intelligence extraction failed at node.');
            }

            return new Response(JSON.stringify({ 
              success: true, 
              data: scrapeData.data,
              message: 'Intelligence synchronized successfully via Master node.' 
            }), { 
              headers: { ...corsHeaders, 'Content-Type': 'application/json' }, 
              status: 200 
            });
